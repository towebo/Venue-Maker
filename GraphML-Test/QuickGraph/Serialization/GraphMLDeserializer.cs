...
namespace QuickGraph.Serialization.Mawingu
{
    ...
    {
        public static Dictionary<string, string> attrmappings = new Dictionary<string, string>();
        ...
        where TGraph : IMutableVertexAndEdgeSet<TVertex, TEdge>
    {
        ...
        static class ReadDelegateCompiler
        {
                ...
                )
            {
                ...
                foreach (var kv in SerializationHelper.GetAttributeProperties(elementType, XmlReaderExtensions.attrmappings))
                {
                ...
                )
            {
                ...
                gen.MarkLabel(next);
                //tmp gen.Emit(OpCodes.Ldloc_0);
                //tmp gen.Emit(OpCodes.Newobj, Metadata.ArgumentExceptionCtor);
                //tmp gen.Emit(OpCodes.Throw);
            ...
            private void ReadGraphHeader()
            {
                while (reader.Read())
                {
                        ...
                        reader.Name == "key")
                    {
                        ...
                        string nme = reader.GetAttribute("attr.name");
                        string _for = reader.GetAttribute("for");

                        if (!string.IsNullOrEmpty(nme))
                        {                            
                            XmlReaderExtensions.attrmappings[nme] = id;

                        }

                        //karl-otto
                        string nils = reader.Name;
